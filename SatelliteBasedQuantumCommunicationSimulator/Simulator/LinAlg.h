...
struct mat4 {
	...
	mat4(vec4 it, vec4 jt, vec4 kt, vec4 lt) {
		rows[0] = it; rows[1] = jt, rows[2] = kt, rows[3] = lt;
	}

	vec4& operator[](int i) { return rows[i]; }
	vec4 operator[](int i) const { return rows[i]; }

	float* getValuePtr() { return &rows[0][0]; }
};
...
inline mat4 RotationMatrix(float angle) {
	return mat4(vec4(cosf(angle), -sinf(angle), 0, 0),
		vec4(sinf(angle), cosf(angle), 0, 0),
		vec4(0, 0, 1, 0),
		vec4(0, 0, 0, 1));
}